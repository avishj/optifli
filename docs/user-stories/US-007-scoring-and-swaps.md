...
4. The engine generates leg-level swap opportunities that are strictly better in at least one ranking dimension and non-worse in all other ranking dimensions, without violating hard constraints; each swap includes impact summary fields (fare delta, duration delta, stop delta, and bundle-compatibility delta).
