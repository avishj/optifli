...
7. Swap impact fields are `fareDelta`, `durationDelta`, `stopDelta`, and `bundleCompatibilityDelta`.
8. Reduced bundle compatibility must trigger user-visible warning about potential total-price increase.

## Deferred Details
1. Exact departure-convenience formula is deferred in docs/TBD.md.
2. Exact computation for `bundleCompatibilityDelta` is deferred until bundle scoring internals are finalized.
...
3. Warning tests verify bundle-compatibility degradation messaging.