...
def _always_hit_adapter(call_count: int) -> FliAdapter:
    ...
    adapter.search = MagicMock(side_effect=[_HIT] * call_count)  # type: ignore[method-assign]  # ty: ignore[invalid-assignment]
    return adapter
...
def _always_miss_adapter(call_count: int) -> FliAdapter:
    mock_client = MagicMock()
    adapter = FliAdapter(client=mock_client)
    adapter.search = MagicMock(side_effect=[_MISS] * call_count)  # type: ignore[method-assign]  # ty: ignore[invalid-assignment]
    return adapter
